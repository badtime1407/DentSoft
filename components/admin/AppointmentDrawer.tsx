        ...
        <div className="px-6 py-4 border-t border-gray-100 shrink-0 space-y-2">
          {mode === 'edit' && appointment?.status === 'PENDING' && onConfirm && (
            <div className="space-y-1.5">
              <button
                type="button"
                disabled={!values.dentistId}
                onClick={() => onConfirm(values.dentistId)}
                className={`w-full px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-all shadow-sm shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed ${focusRing}`}
              >
                ยืนยันนัดหมาย
              </button>
              {!values.dentistId && (
                <p className="text-xs text-amber-600 text-center">กรุณาเลือกทันตแพทย์ก่อนยืนยันนัดหมาย</p>
              )}
            </div>
          )}
          <div className="flex gap-2">
