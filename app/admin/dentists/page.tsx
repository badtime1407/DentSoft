...
import { mockDentists, type MockDentist } from './mock-dentists'
...
type DrawerState =
  ...
  | { open: true; mode: 'edit'; dentist: MockDentist }

export default function AdminDentists() {
  const [dentists, setDentists] = useState<MockDentist[]>(mockDentists)
  ...
  function handleSubmit(values: DentistFormValues) {
    if (drawer.open && drawer.mode === 'edit') {
      ...
      setDentists((prev) => prev.map((d) => (d.id === id ? { ...d, ...values } : d)))
    } else {
      setDentists((prev) => [...prev, { id: `dt-${Date.now()}`, ...values, bookedToday: 0 }])
    }
